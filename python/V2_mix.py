...
Pt_cV, V2cY=numpy.loadtxt('V2_E_AAcY.dat', unpack=True, usecols=[0,1])
Pt_cY, AAcY=numpy.loadtxt('multi_E_AAcY.dat', unpack=True, usecols=[0,1])

#R_AA for electron from B meson decay in Au+Au collision at 62.4GeV
Pt_bV, V2bY=numpy.loadtxt('V2_E_AAbY.dat', unpack=True, usecols=[0,1])
Pt_bY, AAbY=numpy.loadtxt('multi_E_AAbY.dat', unpack=True, usecols=[0,1])
...
for i in range(15):
  ...
  V2.append((V2cY[i]*AAcY[i]*8.3339*10e-2+V2bY[i]*AAbY[i]*2.3765*10e-4)/(8.3339*10e-2*AAcY[i]+2.3765*10e-4*AAbY[i]))   


 
#DataOut= numpy.column_stack((Pt, R_AA)) 

plt.plot(Pt_cY, V2cY, '-', color='green', linewidth=2.5, label='V2 of e from Dmeson, final')
#plt.plot(Pt_cY, AAcN, '-', color='cyan', linewidth=2.5, label='V2 of e from Dmeson, intial')
plt.plot(Pt_bY, V2bY, '-', color='red', linewidth=2.5, label='V2 of e from Bmeson, final')
#plt.plot(Pt_bY, AAbN, '-', color='purple', linewidth=2.5, label='V2 of e from Bmeosn, initial')
...
plt.plot(Pt_cY, V2, '--', color='blue', linewidth=2.5, label='mixed')
plt.xlabel('Pt')
plt.ylabel('V2')
